...
import { Zap, Target, Flame, TrendingUp, Bell, MapPin } from 'lucide-react';
...
export default function Dashboard() {
  const { user, quests, selectedLocation, freeQuestsToday } = useApp();
      ...
      <header className={styles.header}>
        ...
        <div className={styles.headerRight}>
          <button className={styles.notifBtn}>
            <Bell size={20} />
            {showNotif && <span className={styles.notifDot} />}
          </button>
        </div>
      </header>

      {showNotif && (
        <div className={styles.notification} onClick={() => setShowNotif(false)}>
          <MapPin size={16} />
          <span>Yakininda 3 kisi gorev yapiyor!</span>
          <button className={styles.notifClose}>&times;</button>
        </div>
      )}
